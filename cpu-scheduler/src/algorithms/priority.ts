...
export function priorityNonPreemptive(processes: Process[]): SimulationResult {
  ...
  while (completed < n) {
    ...
    // Higher priority number = higher priority; tie-break by arrival then id
    available.sort((a, b) => {
...
export function priorityPreemptive(processes: Process[]): SimulationResult {
  const procs = processes.map(p => ({ ...p, remainingTime: p.burstTime, done: false, completionTime: 0 }));
  const gantt: GanttEntry[] = [];
  const n = procs.length;
  let currentTime = 0;
  let completed = 0;
